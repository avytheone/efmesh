...
- Исполнение — параллельные батчи в пределах модели (`concurrency`, по
  ...
  семафором адаптера. *Межмодельная DAG-конкурентность (модель стартует,
  когда родители досчитали пересекающиеся интервалы) — будущая работа:
  сейчас модели идут в топологическом порядке.*
...
- **F3 — широта.** Postgres-адаптер (libpg_query, state store в PG,
  ...
  Дальше: межмодельная DAG-конкурентность бэкфилла, `target: "ducklake"`
  (§14.5), `init`/`migrate`/`audit`-команды CLI, интерактивное подтверждение
  плана.
