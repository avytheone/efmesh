...
- Исполнение — `Stream` по интервалам с ограниченной конкурентностью,
  ...
  *Оговорка F1: DuckDB-адаптер держит одно соединение, поэтому конкурентность
  вырождается в последовательный топологический порядок — честный параллелизм
  осмыслен с пулом соединений (Postgres, F3).*
...
5. **DuckLake.** Своя раскладка parquet-партиций против DuckLake-каталога:
   ...
   *Проба 2026-07-15: `ATTACH 'ducklake:sqlite:catalog.sqlite'` работает на нашем
   драйвере — снапшоты, time travel `AT (VERSION => n)`, инлайн мелких вставок
   в каталог. SQLite-каталог не добавляет инфраструктуры (мы уже на bun:sqlite),
   но DuckLake — второй владелец истории рядом с нашими fingerprint-версиями
   и учётом интервалов. Реалистичный путь — третья цель материализации
   `target: "ducklake"` (таблица-на-fingerprint), не замена версионности;
   мультипроцессность честно требует Postgres-каталога — это F3.*
