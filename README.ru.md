...
**0.2.1** (beta). Ядро построено и прогнано на живом примере: фазы F0–F6 (см. [SPEC.md §13](https://github.com/avytheone/efmesh/blob/main/SPEC.md) и [CHANGELOG](https://github.com/avytheone/efmesh/blob/main/CHANGELOG.md)), 182 теста, включая живой Postgres-кластер и golden-тесты стабильности fingerprint. Effect v4 — beta-зависимость: пинована точно (peerDependencies), дрейф свежих бет ловит еженедельный CI.
