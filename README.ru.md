...
| `efmesh status <env>` | что происходит: последний план, отставание интервалов, тики run; `--json`, `--check` |
...
форма — JSON-объект с полем `apiVersion` верхнего уровня (сейчас `1`) — единое
целое число, на которое опирается читатель; поднимается только при ломающем
изменении поля, новые поля остаются аддитивными.
...
`status <env> --check` превращает отчёт в health-пробу: выходит с ненулевым
кодом, когда окружение **нездорово** — застрявший бэкфилл (провалённые
интервалы) или последний тик, закончившийся `error`. Нормальные состояния его
не роняют: тики `awaiting-human` / `lock-held`, обычный лаг (тик просто ещё не
догнал) и ни разу не применённое окружение остаются кодом `0`. Отчёт при этом
всё равно печатается, чтобы оператор, которого разбудил алерт, видел причину.

Складывается с запланированным `run` и systemd `OnFailure=`: направьте
обработчик сбоя таймера на проверку, а сервис-«выключатель мертвеца» (например
[healthchecks.io](https://healthchecks.io)) пусть звонит, когда сама проверка
перестаёт отчитываться.

```ini
# efmesh-run@.service — ежечасный тик
[Service]
ExecStart=/usr/bin/env efmesh run %i
OnFailure=efmesh-alert@%i.service      # срабатывает на run с кодом 1

# efmesh-alert@.service — проверить здоровье и пингануть выключатель
[Service]
Type=oneshot
...
Код `2` (`run`, заблокированный структурными изменениями) — не сбой и не
запускает `OnFailure`; он значит, что человек должен сделать `apply` — см.
[коды выхода](#exit-коды).
...
Мажорная версия — `0`, и такой останется надолго: `1.0` будет означать, что
делать больше нечего, а не что мы посерьёзнели. Ниже `1.0` SemVer не обещает
ничего полезного, поэтому вот что обещаем мы:

- **минор (`0.N.0`) вправе сломать** флаги CLI, формы `--json`, whitelist
  публичного API, форму конфига или поверхность определения моделей — всегда
  отдельным пунктом `BREAKING` в [CHANGELOG](https://github.com/avytheone/efmesh/blob/main/CHANGELOG.md), с миграцией в том же пункте;
- **патч (`0.N.M`) не ломает ничего из перечисленного** — только исправления
  дефектов, документация, внутренности, производительность;
- **аддитивное — это минор, а не патч**: новый флаг или новое поле в `--json` —
  это новая функциональность, какой бы мелкой она ни была.

Опираться стоит не на номер пакета, а на контракты со своими версиями:
`apiVersion` в каждом payload `--json`, `STATE_VERSION` (изменение стора
приезжает с `efmesh migrate`), `FINGERPRINT_VERSION` (смена алгоритма
перепланируется как breaking-изменения и пересобирается на следующем apply).
Exit-коды заморожены независимо от версии. Полное правило — [SPEC.md §11.1](https://github.com/avytheone/efmesh/blob/main/SPEC.md).
